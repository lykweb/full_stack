...
```html
  ...
  http://www.w3school.com.cn/html/html5_semantic_elements.asp
	h5兼容性解决方案
	  <!--[if lte IE 8]>
		<script src="路径/html5shiv.min.js"></script>
	  <![endif]-->
```
...
```html
 ...
 ☞ 多媒体标签在网页中的兼容效果方式
	...
	注意：浏览器播放时只能按照一个source标签（按source标签的先后顺序校验，以第一个支持的格式播放，剩余的不管）
```
...
```js
  ☞ 介绍canvas画布
  ☞ 设置画布大小： 使用属性方式设置（使用css直接设置宽高，只是改变canvas标签的宽高，而非画布的宽高）
  ☞ 解决画布重绘问题
  	   1. 设置一次描边
       2. 开启新的图层
```
